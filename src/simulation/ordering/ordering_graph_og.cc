...
void print_ordering_graph_stats_og(ordering_graph const& og) {
  std::size_t edges = 0;
...
ordering_graph::ordering_graph(infra::infrastructure const& infra,
                               tt::timetable const& tt, bool og)
    : ordering_graph(infra, tt, filter{}, og) {}

ordering_graph::ordering_graph(infra::infrastructure const& infra,
                               tt::timetable const& tt, filter const& filter, bool og) {
  ...
  for (auto const& usage_order : orderings) {
    for (auto [from, to] : utl::pairwise(usage_order)) {
      ...
      if(from.id_ == 23258 && to.id_ == 23246) {
        std::cout << "23258 time: " << from.from_.time_since_epoch() << ", 23246 time: " << to.from_.time_since_epoch() << std::endl;
      }
  ...
  print_ordering_graph_stats_og(*this);
}
